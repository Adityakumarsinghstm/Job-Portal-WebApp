...
public class JobDAO {
	...
	public boolean addJobs(Jobs j) {
		boolean f = false;
		try {
			String sql = "insert into jobs(title,category,status,location,description) values(?,?,?,?,?)";
	...
	public boolean updateJob(Jobs j)
	{
		boolean f = false;
		try {
			String sql = "update jobs set title=?,category=?,status=?,location=?,description=? where id=?";
